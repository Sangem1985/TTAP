<%@ Page Title="" Language="C#" MasterPageFile="~/UI/UserMaster.Master" EnableEventValidation="false" AutoEventWireup="true" CodeBehind="frmInspectionRpt.aspx.cs" Inherits="TTAP.UI.Pages.frmInspectionRpt" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function FilterService1(obj, div) {
            var root = 'ContentPlaceHolder1_grdPandM';
            var Filter_Data = $('#' + obj.id).val().toUpperCase();
            if (Filter_Data != "" && Filter_Data != undefined) {
                $("[id*=" + root + "]").each(function (i, j) {
                    var _val = $(this).find('tr td .pmid').html();
                    if (_val.indexOf(Filter_Data) != "0")
                        $(this).show();
                    else
                        $(this).hide();
                });
        ...
        /*function FilterService2(obj, div) {
            var filterResult = $('#' + obj.id).val();
            var empTable = document.getElementById("ContentPlaceHolder1_grdPandM");
            var tr = empTable.getElementsByTagName("tr");
            for (var i = 1; i < tr.length; i++) {
                tr[i].style.display = "none";
                const tdArray = tr[i].getElementsByTagName(".pmid");
                for (var j = 0; j - 1) {
                    tr[i].style.display = "";
                    break;
                }
                ...
                $("#ContentPlaceHolder1_txtSearch").on("keyup", function() {
                    var value = $(this).val().toLowerCase();
                    $("#ContentPlaceHolder1_grdPandM tr td .pmid").filter(function() {
        ...
        function Print() {
            ...
            var divContents = '';
            divContents = document.getElementById("ContentPlaceHolder1_Receipt").innerHTML;
    ...
    <asp:UpdatePanel ID="UpdatePanel4" runat="server">
        <Triggers>
            ...
            <asp:PostBackTrigger ControlID="btnImgNamePlate" />
            <asp:PostBackTrigger ControlID="btnImgPM1" />
            <asp:PostBackTrigger ControlID="btnImgPM2" />
            <asp:PostBackTrigger ControlID="btnImgPM3" />
                                                                    ...
                                                                    <Columns>
                                                                        ...
                                                                        <asp:TemplateField HeaderStyle-CssClass="text-center" HeaderText="Availability of the Machine in Running Condition & Recommended Amount">
                                                                            <ItemTemplate>
                                                                                ...
                                                                                <asp:TextBox ID="txtAdmissibleAmount" Visible="true" Enabled="false" Text='<%#Eval("DLORecommendedMachineCost") %>' Style="height: 34px; width: 190px;" class="form-control" OnTextChanged="txtAdmissibleAmount_TextChanged" AutoPostBack="true" onkeypress="return DecimalOnly(event)" runat="server" Height="100px"></asp:TextBox>
                                                                                <asp:Label ID="lblremarks" runat="server"> Remarks : </asp:Label>
                                                                                <asp:TextBox ID="txtremarks" Visible="true" Text='<%#Eval("Remarks") %>' class="form-control" TextMode="MultiLine" runat="server" Height="100px"></asp:TextBox>
                                                                            </ItemTemplate>
                                                                            <HeaderStyle CssClass="text-center" />
                                                                            <ItemStyle HorizontalAlign="Left" CssClass="text-center" Width="280px" />
                                                                        </asp:TemplateField>
